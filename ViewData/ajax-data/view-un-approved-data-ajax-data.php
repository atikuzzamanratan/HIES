...
if (!empty($_POST)) {
	...
	$qry = "SELECT xfr.id, 
					...
					(
						...
						FROM masterdatarecord_UnApproved mdp 
    ...
    foreach ($resQry as $row) {
        ...
        //$encrypRecordID = encryptValue($RecordID . '|||' . $LoggedUserID . '|||' . $UserID);
        //$encrypRecordID = encryptValue($RecordID);
        ...
        $actions .= "<button title=\"$btnTitleNotice\" type=\"button\" class=\"btn btn-outline-secondary\" style=\"display: inline-block;margin: 0 1px;\" data-bs-toggle=\"modal\" data-bs-target=\"#sendNoticeModal$RecordID\"><i class=\"fas fa-bell\"></i></button>
                ...
                <script type=\"text/javascript\">
                    function ShowDataDetail(DataFromID, recordID, isAproved, psu, loggedUserID, agentID, XFormsFilePath, data) {
                            $.ajax({
                                url: 'ViewData/ajax-data/data-detail-view-unapproved-data.php',
                                method: 'GET',
                                datatype: 'json',
                                data: {
                ...
                <!-- View Data Modal-->
                <div class=\"modal fade bd-example-modal-lg\" id=\"viewDataModal\" tabindex=\"-1\" aria-labelledby=\"viewDataModalLabel\" aria-hidden=\"true\">
        ...
        $actions .= "<!-- Edit Data Modal-->
                <script type=\"text/javascript\">
                    function EditDataDetail(recordID, isAproved, psu, loggedUserID, agentID, XFormsFilePath, data) {
                            $.ajax({
                                url: 'ViewData/ajax-data/data-edit-view-unapproved-data.php',
                                method: 'GET',
                                datatype: 'json',
                                data: {
                                    id: recordID,
                                ...
                                success: function (response) {
                                    //alert(response);
                                    $('#dataEditViewDiv').html(response);
                                }
                            }); 
                        return false;
                    }
                </script>
                <div class=\"modal fade bd-example-modal-lg\" id=\"editDataModal\" tabindex=\"-1\" aria-labelledby=\"editDataModalLabel\" aria-hidden=\"true\">
                  <div class=\"modal-dialog modal-lg\">
                    <div id=\"dataEditViewDiv\" class=\"modal-content\">
                    </div>
                  </div>
                </div>";
